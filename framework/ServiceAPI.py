...
class ServiceAPI():
    ...
    def get_module(self,product):  # 获取项目下的模块
        sql="select * from  zt_module WHERE root=%s and (type='bug' or type='story')  and deleted='0' ;"%(product['product'])
        logger.info(sql)
        ...
        data={'message': '操作成功', 'result_code': '0000', 'data':{"modulelist":data,'Modulename':diclist} }
        logger.info(data)
        return data
    ...
    def module_info(self,product):#各个模块详情
        ...
        for n in projects_info['data']['modulelist']:
            ...
            sql = "select count(*) from  zt_bug WHERE product=%s and deleted='0' And module=%s ;" % (product['product'],n)
            logger.info(sql)
            dic.update({'sums': Query_DB().getnum(sql)})
            status_list = ['closed', 'active', 'resolved']
            for i in status_list:
                sql = "select count(*) from  zt_bug WHERE product=%s and deleted='0' And module=%s AND status='%s';" % (product['product'],
                    n, i)
                data = Query_DB().getnum(sql)

                dic.update({i: data})
            for i in severity():
                sql = "select count(*) from  zt_bug WHERE product=%s and deleted='0' and status!='closed' And module=%s AND severity=%s;" % (product['product'],
                    n, i)
                data = Query_DB().getnum(sql)
    ...
    def get_module_bug(self,module):#获取模块未关闭BUG列表
        ...
        sql='select * from  zt_bug WHERE module=%s and deleted="0" and status!="closed" order  by severity asc,pri asc;'%(module['module'])
        logger.info(sql)
    ...
    def get_product_module_sum(self,dic):#获取项目近况数据
        ...
        if dic['module'] == "" and dic['product'] != '':
            sql_product = "select * from  zt_product WHERE id=%s ;" % (dic['product'])
            sql_all = 'select count(*) from  zt_bug WHERE product=%s and deleted="0";' % (dic['product'])
            logger.info('sql_product：%s'%sql_product)
            logger.info('sql_all：%s' % sql_all)
            dic_data.update(
